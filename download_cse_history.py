...
def find_market_watch_items(obj: Any) -> Optional[List[dict]]:
    """
    Recursively search for the market watch list.
    Handles:
    - list of market watch rows
    - {"values": [...]}
    - {"data": [...]}
    - {"data": {"values": [...]}}
    - other nested wrappers
    """
    if looks_like_market_watch_list(obj):
        return obj

    if isinstance(obj, dict):
        # direct common cases first
...
def build_ticker_id_map_from_market_watch(payload) -> Dict[str, int]:
    """
    Expects market watch objects with:
    - ticker
    - field_symbol (internal instrument id)
    """
...
def build_history_params(symbol_id: int, start_date: str, offset: int, limit: int) -> dict:
    """
    Mirrors the successful captured request structure.
    """
    return {
...
def fetch_full_history_for_ticker(
    ...
    while True:
        ...
        for url in candidate_urls:
            try:
                payload = safe_get_json(session, url, params=params)
                preferred_url = url
                break
        ...
        if payload is None:
            raise RuntimeError(
                f"Request failed for {ticker} (id={symbol_id}) at offset={offset}. "
...
def main() -> None:
    ...
    if missing:
        ...
        print(", ".join(missing))
        print("\nAdd them to MANUAL_TICKER_ID_MAP or provide a more complete market_watch.json.")
        return
